...
struct ParameterSpec{SpecType}
    ...
    check  # Hard constraint with error, evaluate on model:    (val, model) -> Bool
    warn    # Soft constraint with warning, evaluate on model:  (val, model) -> Bool
    val
    info::String
end

const OwnedParameter = ParameterSpec{:Owned}
const DerivedSpec = ParameterSpec{:Derived}

OwnedParameter(;type, default, validation = x -> true, info = "", kwargs...) = ParameterSpec{:Owned}(;type, default, validation, info, kwargs...)
...
function construct(spec::ParameterSpec{T}, model) where T
    if T == :Owned #Already validated, just return the value
        if isnothing(spec.val)
            return spec.default
        return spec.val
    elseif T == :Derived # Need to validate and construct from model
        i
end
...
struct Internal{NT} <: InternalImplementation
    nt::NT
end

function Base.getproperty(ht::HamiltonianTerm, name::Symbol)
    if hasfield(ht, name)
        return getfield(ht, name)
    elseif hasfield(ht, :parameters) # If the HamiltonianTerm has a parameters field, check if the name is in there
...
function parse_hamiltonian_parameters(terms...)
    ...
    owned_params = (;)
    while true # Loop until no more OwnedParameter are found
        el, terms = type_parse(OwnedParameter, terms...; default = nothing, error = false)
        if isnothing(el)
            break
        else
