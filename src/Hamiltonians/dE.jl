...
function expr_dEIsing(stype::Type{<:SType}) 
    ...
    expr = "begin
        efactor = zero(T)
        @turbo check_empty = $(check_empty[]) for ptr in @ptrs
...
dEIsing(g, idx) = dEIsing(g, state(g), adj(g), idx, stype(g)) 
