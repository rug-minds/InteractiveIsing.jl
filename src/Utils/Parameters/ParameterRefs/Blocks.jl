...
function generate_block(rr::RefReduce, argstype, idxs = (;), precision = nothing, first = true)
    debugmode = argstype isa NamedTuple
    if debugmode
        println("generate_block refreduce: $(rr)")
    end

    # bmodel = get_blockmodel(rr, argstype, idxs)
    # println(bmodel)
    # return bmodel(rr, argstype, idxs)
    ...
    if ispure(rr) #Treat them all like a sum in turbo, small optimisation if possible
        ...
        exps = quote
            $(assignments...)
            $(nested_turbo_wrap(total_add, (:(axes($(vec_names[1]), $i_ind)) for i_ind in 1:length(contract_ind)) |> collect, contract_ind))
