...
function generate_plan_context_step(plan::P, this_plan_wiring::W = getwiring(plan), namespaces::N = getfield(plan, :namespaces)) where {P<:Union{CompositeAlgorithm, Routine}, W<:PlanWiring, N<:Tuple}
    ...
    funcbody = quote
        function (
            ...
            _process::Proc,
            _lifetime::LT,
        ) where {Plan<:$(typeof(plan)), C<:ProcessContext, Proc<:AbstractProcess, LT<:Lifetime}
            $(plan_prelude...)
            $(child_exprs...)
            $(plan_epilogue...)
            return _context
...
function generate_root_context_step(la::LA) where {LA<:LoopAlgorithm}
    ...
    funcbody = quote
        function (
            ...
            _process::Proc,
            _lifetime::LT,
        ) where {Algo<:AbstractLoopAlgorithm, C<:ProcessContext, Proc<:AbstractProcess, LT<:Lifetime}
