...
registry attached.
"""
@inline function resolve(la::LA) where {LA<:LoopAlgorithm}
    if isresolved(la)
        return la
    end
...
@inline function setup_registry_and_keyed_algos(la::LA) where {LA<:LoopAlgorithm}
...
@inline function add_algos_to_registry(registry::R, la::LA, multiplier) where {R<:NameSpaceRegistry, LA<:LoopAlgorithm}
    funcs = getalgos(la)
...
@inline function add_algo_to_registry(registry::R, algo::LA, multiplier) where {R<:NameSpaceRegistry, LA<:LoopAlgorithm}
...
@inline function rebuild_loopalgorithm_funcs(la::LA, funcs::F) where {LA<:LoopAlgorithm, F<:Tuple}
...
@inline function attach_registry_to_tree(la::LA, registry::R) where {LA<:LoopAlgorithm, R<:NameSpaceRegistry}
...
@inline function _resolve_keyed_with_registry(la::LA, registry::R, options::O) where {LA<:LoopAlgorithm, R<:NameSpaceRegistry, O<:Tuple}
...
@inline function _resolve_options(la::LA) where {LA<:LoopAlgorithm}
    if isresolved(la)
