...
using InteractiveIsing.StatefulAlgorithms # legacy combined experiment
...
function reference_coulomb_scale(g, depol_term, P0)
    return with_saved_state(g, depol_term) do
        ...
        for i in eachindex(state(g))
            ...
            local_field = abs(InteractiveIsing.calculate(InteractiveIsing.d_iH(), depol_term, g, proposal))
...
# Mobile charged defects, integrated from examples/3D Coulomb Defect Hopping.jl.
# These values use Basefile's existing dimensionless energy/length convention.
vacancy_count = 10
electron_count = 20
vacancy_charge = 2.0f0
electron_charge = 1.0f0
...
nspins = xL * yL * zL
net_free_charge = vacancy_count * vacancy_charge - electron_count * electron_charge
iszero(net_free_charge) || error(
    "Coulomb defect hopping requires neutral free charge: " *
    "vacancy_count * vacancy_charge must equal electron_count * electron_charge. " *
    "Current net free charge = $net_free_charge.",
)
...
g = IsingGraph(xL, yL, zL, 
        ...
        # Ising(b = UniformArray(0), localpotential = coeff2) + 
            ...
            CoulombHamiltonian(
                ...
                free_charge_split = free_charge_split,
            ) +
...
vacancy_hamiltonian =
    CoulombChargeCoupling(vacancy_charge; split = free_charge_split) +
    ExternalFieldChargeCoupling() +
    LocalPotentialShiftCoupling(2, vacancy_quadratic_shift) +
    LocalPotentialShiftCoupling(4, vacancy_quartic_shift)

electron_hamiltonian =
    CoulombChargeCoupling(-electron_charge; split = free_charge_split) +
    ExternalFieldChargeCoupling()
...
charges = DefectsModel(
    ...
    rng = MersenneTwister(defect_rng_seed),
)

# Optional live view of the two mobile species:
# interface(
#     charges;
...
# ---- Start simulation2 ----
createProcessManual(g, Pulse_and_Relax,
...
run_params = params_dataframe(
    ...
    "vacancy_charge" => vacancy_charge,
    "electron_charge" => electron_charge,
    "electron_attempt_rate" => electron_attempt_rate,
    "defect_step_interval" => defect_step_interval,
    "free_charge_split" => free_charge_split,
    "net_free_charge" => net_free_charge,
    "vacancy_quadratic_shift" => vacancy_quadratic_shift,
    "vacancy_quartic_shift" => vacancy_quartic_shift,
    "defect_rng_seed" => defect_rng_seed,
    "Amp1" => Amp1,
    "nrepeats" => nrepeats,
