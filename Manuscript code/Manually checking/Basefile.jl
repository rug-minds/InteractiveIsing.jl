...
Spatial Landau disorder / pinned dipoles / domains:
    In the decoupled / independent localpotential form, each Landau
    coefficient should be a state-length vector. The vector order follows
    Julia's vec(coeff3d), so a temporary 3D mask can still be useful when
    building the vector.
...
#=
Proposal scale vs Langevin stepsize

    proposal_delta
        Used by Metropolis / proposer-style updates.
        If it is a number, the graph receives LocalProposer(proposal_delta).
        This controls how far one trial proposal jumps.

            proposal_delta = 0.1
            proposer_args = (LocalProposer(proposal_delta),)
            metropolis = g.default_algorithm

        If it is nothing, the graph uses the library default proposer.

            proposal_delta = nothing
            proposer_args = ()

    stepsize
        Used by Langevin algorithms, not by LocalProposer.
        This controls the Langevin drift/noise update scale.

            dynamics = LocalLangevin(; stepsize = 0.05f0, adjusted = true)
            dynamics = GlobalLangevin(; stepsize = 0.01f0, adjusted = false)
            dynamics = BlockLangevin(; stepsize = 0.02f0, block_size = 128, adjusted = false)

    Practical rule:
        Metropolis: tune proposal_delta.
        LocalLangevin / GlobalLangevin / BlockLangevin: tune stepsize.
        Do not compare proposal_delta and stepsize as if they were the same
        physical parameter; they are both scales, but for different update rules.

    In the Manuscript helper scripts this is represented by:
        proposal_delta = 0.1              # Metropolis / LocalProposer
        algorithm_name = :local_langevin
        algorithm_kwargs = (; stepsize = 0.05f0, adjusted = true)

    Basefile selector examples:
