...
outdir = raw"D:\Code\data\Manuscript\domians\shell_ferro"
mkpath(outdir)
# ---- parameters to sweep ----
JIsing = 1.0
Scale = 1
...
coeff2_disorder_scale = 0.02f0
coeff4_disorder_scale = 0.05f0
coeff6_disorder_scale = 0.05f0
coeff8_disorder_scale = 0.02f0
coeff10_disorder_scale = 0.02f0
...
fStepDual = let
    f = Figure()
    ax_left = Axis(f[1, 1], xlabel = "Step", ylabel = "Pr")
    ax_right = Axis(f[1, 1], ylabel = "H_field", yaxisposition = :right)
    ...
    steps = eachindex(Pr2)
    lines!(ax_left, steps, Pr2, color = :blue, label = "Pr")
    lines!(ax_right, steps, Hfield2, color = :red, label = "H_field")
...
# fHrestPr = makieaxis(
#     f -> Axis(f[1, 1], xlabel = "Pr", ylabel = "H without Field"),
#     ax -> lines!(ax, Pr2, Hrest2)
# )

# fHrestV = makieaxis(
#     f -> Axis(f[1, 1], xlabel = "Voltage", ylabel = "H without Field"),
#     ax -> lines!(ax, voltage2, Hrest2)
# )

fHrestV = makieaxis(
    f -> Axis(f[1, 1], xlabel = "Pr", ylabel = "H_poly"),
    ax -> lines!(ax, Pr2, Hpoly2)
)

fHrestV = makieaxis(
    f -> Axis(f[1, 1], ylabel = "H without Field"),
    ax -> lines!(Hrest2)
)

fHrestV = makieaxis(
    f -> Axis(f[1, 1], ylabel = "H Field"),
    ax -> lines!(Hfield2)
)
