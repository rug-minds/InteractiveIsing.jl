...
function normalize_adj_by_average_col(adj::A, scaling = one(eltype(adj))) where A
    ...
    println("Average column sum before normalization: ", avg_col_sum)
...
wg0 = @WG weightfunc_xy_dilog_antiferro NN = (2,2,2)
wg1 = @WG weightfunc1
wg2 = @WG weightfunc1 NN = (3,3,3)
...
outdir = raw"D:\Code\data\20260224\Normalize"
mkpath(outdir)

# Run a sweep without re-running the first two setup cells.
# This cell will reconfigure the Hamiltonian, run the process, and SAVE (PNG + XLSX) for each scale.
...
adj=g.adj
newadj = normalize_adj_by_average_col(adj, 8f0)
g.adj = newadj
...
return (; Scrn = Float64(Screening), png1_path, png2_path, png_path_dist, xlsx_path)
...
# T = 1
# Scale = 4
# Scrn_val = 0.1, 1, 2, 5, 10, 15, 20, 25, 30, 40, 50

# # ---- loop over Scale ----
# results = Dict{Float64, Any}()
# for Scrn in Scrn_val
...
# end
# println("Done. Available keys(Scrn) = ", collect(keys(results)))