...
g.hamiltonian = Ising(g) + DepolField(g) + Quartic(g) + Sextic(g)
g.hamiltonian = sethomogeneousparam(g.hamiltonian, :b)
...
Metro_with_oldAlgr = CompositeAlgorithm(metropolis, M_Logger, B_Logger,
    (1, point_repeat, point_repeat),
    Route(metropolis => M_Logger, :M => :value),
    Route(metropolis => B_Logger, :hamiltonian => :value, transform = x -> x.b[]),
)

pulse_part1 = CompositeAlgorithm(Metro_with_oldAlgr, pulse1, (1, point_repeat))

Pulse_and_Relax = Routine(pulse_part1, Metro_with_oldAlgr,
