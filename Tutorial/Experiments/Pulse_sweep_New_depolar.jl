...
outdir = raw"C:\Users\P317151\Documents\data\Model_V1.0\20260217\Pulse sweep\Diff_scale"
mkpath(outdir)
...
function run_simu!(g; Scale = 1, Screening = 0.1, Amp1 =20, Temp = 0.3, time_fctr = 1, Steps_1 = 4000)
    g.hamiltonian = Ising(g, :homogeneous_b) + CoulombHamiltonian(g, Scale, screening = Screening, recalc = 1000) + Quartic(g) + Sextic(g)
    # Landau/self terms
    ...
    capture_interval = pulse_time/(nrepeats*8) # capture every segment of the pulse
    ...
    createProcess(g, Pulse_and_Relax, lifetime = 1, Input(Graph_Logger, filepath = raw"C:\Users\P317151\Documents\data\Model_V1.0\20260217\Pulse sweep\Diff_scale"))
    ...
    base_name = string(
        "_Scale=", round(Scale, digits = 6),
        "_Screening=", round(Screening, digits = 4),
        "_Amp=", round(Amp1, digits = 4),
    ...
    return (; Scale = Float64(Scale), png1_path, png2_path, png_path_dist, xlsx_path)
end
...
T = 1
Scrn = 10
Scale_val = (0, 0.1, 0.2, 0.5, 1.0, 2.0, 3.0 ,5.0)


# ---- loop over Scale ----
results = Dict{Float64, Any}()
for Scale in Scale_val
    @info "Running sweep" Scale
    res = run_simu!(g; Scale = Float64(Scale), Screening=Scrn, Amp1 = Amptitude, Temp = T, time_fctr = 1, Steps_1 = 4000)
    results[Float64(Scale)] = res

end
println("Done. Available keys(Scale) = ", collect(keys(results)))