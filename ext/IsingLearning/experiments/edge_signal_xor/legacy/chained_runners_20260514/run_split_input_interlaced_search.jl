...
const INTERLACED_INPUT_SCALE_REF = Ref{FT}(0.75)

"""
    interlaced_edge_input_weight(; c1, c2)

Return a deterministic input-to-left-edge coupling for the interlaced edge map.

Input spin 1 drives odd rows of the hidden layer's first edge (`1,3,5,7`).
Input spin 2 drives even rows (`2,4,6,8`). This keeps both input bits present
along the whole input edge instead of separating them into top/bottom halves.
"""
function interlaced_edge_input_weight(; c1, c2)
    ...
    if (input_row == 1 && row_is_odd) || (input_row == 2 && !row_is_odd)
        return INTERLACED_INPUT_SCALE_REF[]
    end
    return zero(FT)
end

"""Create the interlaced input-to-left-edge weight generator."""
function interlaced_edge_input_generator(scale::FT)
    INTERLACED_INPUT_SCALE_REF[] = scale
    return @WG interlaced_edge_input_weight NN = :all symmetric = true
end

"""Build the interlaced-input `2 -> 8x8 -> 1` edge graph."""
function interlaced_edge_graph(config::EdgeSignalXORConfig)
    ...
    input_hidden = interlaced_edge_input_generator(config.input_hidden_scale)
...
"""Wrap the interlaced edge graph in the standard learning layer."""
function interlaced_edge_layer(graph, config::EdgeSignalXORConfig)
    dynamics = edge_sampler(config)
    return LayeredIsingGraphLayer(
        () -> interlaced_edge_graph(config);
...
"""Initialize an interlaced-edge trainer with optional nudged-temperature bump."""
function interlaced_edge_trainer(config::EdgeSignalXORConfig, nudged_temp_factor::FT)
    graph = interlaced_edge_graph(config)
    layer = interlaced_edge_layer(graph, config)
...
"""Train one interlaced-edge graph with time-averaged validation."""
function train_interlaced_edge_xor(
    ...
    trainer = interlaced_edge_trainer(config, nudged_temp_factor)
...
"""Return interlaced-edge candidate settings."""
function interlaced_edge_specs()
    common = (;
        epochs = 9000,
        log_every = 500,
        minit = 4,
    ...
    raw = [
        (; name = "interlaced_b0p50_T0p010_eta1p0_lr20e4_wd5e3", β = FT(0.50), temp_fraction = FT(0.010), stepsize = FT(1.0), lr = FT(0.00020), wd = FT(0.005), bump = FT(1.30), seed = 1_250_001),
        (; name = "interlaced_b0p35_T0p010_eta1p0_lr25e4_wd4e3", β = FT(0.35), temp_fraction = FT(0.010), stepsize = FT(1.0), lr = FT(0.00025), wd = FT(0.004), bump = FT(1.25), seed = 1_250_101),
        (; name = "interlaced_b0p35_T0p007_eta1p2_lr20e4_wd4e3", β = FT(0.35), temp_fraction = FT(0.007), stepsize = FT(1.2), lr = FT(0.00020), wd = FT(0.004), bump = FT(1.35), seed = 1_250_201),
    ]
    specs = []
    for spec in raw
...
"""Run the interlaced-edge input search."""
function run_interlaced_edge_search(; rootdir = joinpath(@__DIR__, "runs", "edge_split_input_interlaced_" * Dates.format(now(), "yyyymmdd_HHMMSS")))
    mkpath(rootdir)
    rows = Dict{String,Any}[]
    eval_burnin_sweeps = 120
    eval_average_sweeps = 120
    for spec in interlaced_edge_specs()
        println("\n=== ", spec.name, " ===")
        ...
        mkpath(outdir)
        trained = train_interlaced_edge_xor(
            spec.config;
            outdir,
    ...
    println("Saved interlaced-edge run: ", rootdir)
    return rootdir
end

if abspath(PROGRAM_FILE) == @__FILE__
    run_interlaced_edge_search()
end
