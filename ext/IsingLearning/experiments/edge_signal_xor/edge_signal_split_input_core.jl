...
function run_dynamics_steps!(graph, sampler, nsteps::Integer; seed::Integer)
    ...
    process = Processes.Process(routine, inputs...; repeats = 1)
    run(process)
    wait(process)
    close(process)
    return graph
...
Source-only file for split-input edge XOR experiments. Run files should include
this file and provide configurations. This file intentionally does not start a
run when executed through `include`.
"""
...
Return an input-to-hidden weight for the split-edge input architecture.

Input spin 1 connects only to the upper half of the first hidden edge. Input
spin 2 connects only to the lower half. This keeps the input dimension at two
while giving the two input bits separate physical entry points.
"""
...
"""Create the split input-to-left-edge weight generator."""
...
Build the fixed split-edge architecture:
...
"""Wrap the split-edge graph in the standard learning layer interface."""
...
"""Initialize the existing threaded trainer for the split-edge architecture."""
...
"""Build plus/minus nudged phases with a temporary nudged-temperature multiplier."""
function split_input_nudged_temp_algorithm(layer, base_temp::FT, nudged_temp::FT)
    ...
    final = Processes.@CompositeAlgorithm begin
        ...
        nudged()
        plus_capture(isinggraph = nudged.dynamics.model)
        nudged.nudged_beta = @transform(x -> -x, clamping_beta)
        nudged()
...
"""Initialize a split-edge trainer with optional nudged-only temperature bump."""
...
"""Train one split-edge graph using the existing trainer path."""
...
"""Return the active non-input spin count for converting sweeps to steps."""
...
"""Build a split-edge config from free/nudged/validation sweep counts."""
function split_input_config_from_sweeps(;
    ...
    base_seed::Int,
    hidden_nn::Int = 5,
    input_hidden_scale::FT = FT(0.45),
    hidden_local_scale::FT = FT(0.005),
    hidden_output_scale::FT = FT(0.40),
    bias_scale::FT = FT(0.0),
    temp_fraction::FT = FT(0.018),
    stepsize::FT = FT(0.8),
)
    ...
    return (; config, active_count, free_sweeps, nudged_sweeps, validation_sweeps)
end
