...
right_hidden_edge(c, hidden_width::Integer = 8) = c[2] == hidden_width
...
function response_indices(graph)
    ...
    hidden_matrix = reshape(hidden_range, 8, 8)
...
function log_response!(trace::ResponseTrace, graph, pre_state; graph_kind, nn, source_case, target_case, repeat_idx, sweep)
    ...
    s = II.state(graph)
    Δhidden = reshape(s[idxs.hidden_range] .- pre_state[idxs.hidden_range], 8, 8)
