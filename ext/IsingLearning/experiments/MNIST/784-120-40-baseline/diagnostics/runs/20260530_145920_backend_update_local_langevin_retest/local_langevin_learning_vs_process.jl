...
function main()
    ...
    repeats = parse(Int, get(ENV, "ISING_MNIST_LL_BENCH_REPEATS", "5"))
    rows = NamedTuple[]
    ...
    for repeat_idx in 1:repeats
        ...
        append_langevin_learning_row!(row)
        push!(rows, row)
        println(row)
    end

    direct_ratios = map(row -> row.direct_bespoke_seconds_per_example, rows)
    process_ratios = map(row -> row.serial_process_seconds_per_example, rows)
    manager_ratios = map(row -> row.manager_seconds_per_example, rows)
    process_over = map(row -> row.process_over_bespoke, rows)
    manager_over = map(row -> row.manager_over_bespoke, rows)

    summary = (;
        repeats,
        direct_mean = mean(direct_ratios),
        direct_median = median(direct_ratios),
        process_mean = mean(process_ratios),
        process_median = median(process_ratios),
        manager_mean = mean(manager_ratios),
        manager_median = median(manager_ratios),
        process_over_bespoke_mean = mean(process_over),
        process_over_bespoke_median = median(process_over),
        manager_over_bespoke_mean = mean(manager_over),
        manager_over_bespoke_median = median(manager_over),
    )
    println(summary)
    println("csv=" * joinpath(LANGEVIN_LEARNING_DIR, "local_langevin_learning_vs_process.csv"))
    return (; rows, summary)
end
