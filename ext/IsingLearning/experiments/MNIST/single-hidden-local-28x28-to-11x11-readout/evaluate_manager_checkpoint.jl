...
"""Load serialized manager parameters and rebuild a matching model."""
function load_manager_checkpoint(
    path::P;
    test_per_class::I,
    free_reads::J,
    free_sweeps::K,
...
"""Write a compact one-row CSV for one checkpoint evaluation."""
...
"""Evaluate one saved manager checkpoint on a balanced MNIST test slice."""
function main()
    ...
    outdir = get(
        ...
        joinpath(@__DIR__, "experiments", "current", "checkpoint_eval_" * Dates.format(now(), "yyyymmdd_HHMMSS")),
    )

    model = load_manager_checkpoint(
        checkpoint;
        test_per_class,
        free_reads,
        free_sweeps,
