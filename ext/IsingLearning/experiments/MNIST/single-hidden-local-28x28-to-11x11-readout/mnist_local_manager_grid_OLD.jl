...
function sampled_graph(
    ...
    base_bias = isnothing(shared_base_bias) ? (g -> II.filltype(Vector, 0f0, II.statelen(g))) : II.Force(shared_base_bias)
...
function evaluate(model::M, x::X, y::Y) where {M<:LocalMNISTModel,X<:AbstractMatrix,Y<:AbstractMatrix}
    ...
    eval_meter = progress_meter(config, size(x, 2), "eval $(config.name): ")
    try
        for sample_idx in axes(x, 2)
            ...
            tick_progress!(eval_meter, [(:sample, sample_idx), (:accuracy, round(correct / sample_idx; digits = 4))])
        end
    finally
        finish_progress!(eval_meter)
    end
