...
$root = Join-Path $diag "observable_symmetric_asym_sweeps_grid"
...
function Invoke-AsymSweepRun {
    ...
    $name = "r8_obs_sym_b$(Format-Token $Beta)_free${FreeBurn}p${FreeAvg}_nudge${NudgeBurn}p${NudgeAvg}_lr$(Format-Token $LearningRate)_e60"
    ...
    if (-not (Test-Path $manifest)) {
        "timestamp,name,beta,free_burn,free_avg,nudge_burn,nudge_avg,lr_w,cold_T,reverse_T,outdir" | Out-File -FilePath $manifest -Encoding utf8
    }
    "$(Get-Date -Format s),$name,$Beta,$FreeBurn,$FreeAvg,$NudgeBurn,$NudgeAvg,$LearningRate,0.003,0.3,$outdir" | Out-File -FilePath $manifest -Append -Encoding utf8

    $stdout = Join-Path $logs "$($name)_stdout.log"
    $stderr = Join-Path $logs "$($name)_stderr.log"
    "[$(Get-Date -Format s)] START $name beta=$Beta free=$FreeBurn/$FreeAvg nudge=$NudgeBurn/$NudgeAvg lr=$LearningRate" | Tee-Object -FilePath $stdout -Append
    ...
    if ($exit -ne 0) {
        throw "Observable asymmetric-sweeps run $name failed with exit code $exit"
    }
}

$runs = @(
    @{ Beta = "5.0";  FreeBurn = "10"; FreeAvg = "3"; NudgeBurn = "50"; NudgeAvg = "25"; LearningRate = "0.00001" },
    @{ Beta = "10.0"; FreeBurn = "10"; FreeAvg = "3"; NudgeBurn = "50"; NudgeAvg = "25"; LearningRate = "0.00001" },
    @{ Beta = "15.0"; FreeBurn = "10"; FreeAvg = "3"; NudgeBurn = "50"; NudgeAvg = "25"; LearningRate = "0.00001" },
    @{ Beta = "5.0";  FreeBurn = "5";  FreeAvg = "1"; NudgeBurn = "75"; NudgeAvg = "50"; LearningRate = "0.000005" },
    @{ Beta = "10.0"; FreeBurn = "5";  FreeAvg = "1"; NudgeBurn = "75"; NudgeAvg = "50"; LearningRate = "0.000005" }
)
