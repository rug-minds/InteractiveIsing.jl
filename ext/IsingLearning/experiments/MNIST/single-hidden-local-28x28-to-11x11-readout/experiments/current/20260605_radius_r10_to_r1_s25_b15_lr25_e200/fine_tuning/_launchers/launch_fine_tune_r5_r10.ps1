...
$fine = Join-Path $series "fine_tuning"
$logdir = Join-Path $fine "logs"
...
foreach ($branch in $branches) {
    foreach ($baseRun in $baseRuns) {
        ...
        $targetEpochs = [int]$baseRun.TargetEpochs
        $sourceCheckpoint = [string]$baseRun.SourceCheckpoint
        $outRoot = Join-Path $fine $branch.Name
...
Add-Content -Path $status -Value "[$(Get-Date -Format s)] FINE TUNING COMPLETE"
