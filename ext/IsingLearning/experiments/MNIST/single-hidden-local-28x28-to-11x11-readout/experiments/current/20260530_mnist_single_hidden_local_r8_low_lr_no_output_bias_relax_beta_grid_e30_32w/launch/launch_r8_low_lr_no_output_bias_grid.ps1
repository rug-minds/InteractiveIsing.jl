...
$Root = Join-Path $Repo "ext\IsingLearning\experiments\MNIST\single-hidden-local-28x28-to-11x11-readout\experiments\current\20260530_mnist_single_hidden_local_r8_low_lr_no_output_bias_relax_beta_grid_e30_32w"
...
foreach ($sweep in $Sweeps) {
    foreach ($beta in $Betas) {
        $name = "r8_s${sweep}_beta$($beta.Label)_e30_low_lr_nooutbias"
