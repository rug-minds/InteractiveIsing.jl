...
"""Append one high-sweep grid summary row."""
...
"""Run one high-sweep grid point and append root-level status."""
function run_grid_point!(sweeps::I, radius::J, beta::T) where {I<:Integer,J<:Integer,T<:Real}
    name = "r$(Int(radius))_beta$(beta_label(beta))_e30"
    ...
    println("[", Dates.format(now(), "yyyy-mm-ddTHH:MM:SS"), "] high-sweep run started sweeps=", sweeps, " radius=", radius, " beta=", beta)
    flush(stdout)
    started = time()
    try
        ...
        println("[", Dates.format(now(), "yyyy-mm-ddTHH:MM:SS"), "] high-sweep run finished sweeps=", sweeps, " best_accuracy=", result.best_accuracy)
        flush(stdout)
        return result
    catch err
        mkpath(outdir)
        ...
        println("[", Dates.format(now(), "yyyy-mm-ddTHH:MM:SS"), "] high-sweep run failed sweeps=", sweeps, " error=", err)
        flush(stdout)
        return nothing
    end
end

"""Run the high-sweep grid sequentially."""
function main()
    mkpath(EXP_ROOT)
    sweeps_grid = parse_grid_ints("ISING_MNIST_HIGH_SWEEPS", (150, 250, 400, 800, 1200))
    radius = parse(Int, get(ENV, "ISING_MNIST_HIGH_SWEEP_RADIUS", "8"))
    beta = parse(PMNIST_FT, get(ENV, "ISING_MNIST_HIGH_SWEEP_BETA", "5.0"))
    println("[", Dates.format(now(), "yyyy-mm-ddTHH:MM:SS"), "] high-sweep grid launcher started root=", EXP_ROOT)
    println("[", Dates.format(now(), "yyyy-mm-ddTHH:MM:SS"), "] threads=", Threads.nthreads(), " sweeps=", sweeps_grid, " radius=", radius, " beta=", beta)
    flush(stdout)

    for sweeps in sweeps_grid
        mkpath(joinpath(EXP_ROOT, "s$(Int(sweeps))"))
        run_grid_point!(sweeps, radius, beta)
        GC.gc()
    end
    println("[", Dates.format(now(), "yyyy-mm-ddTHH:MM:SS"), "] high-sweep grid launcher finished")
    flush(stdout)
    return nothing
end
