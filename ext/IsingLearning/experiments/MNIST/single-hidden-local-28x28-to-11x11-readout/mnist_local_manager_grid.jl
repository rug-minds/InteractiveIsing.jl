...
"""Initialize trainable local-MNIST parameters and install graph couplings."""
...
"""Load a serialized manager checkpoint and install compatible parameters."""
function resume_model!(model::M, path::P) where {M<:PaperMNISTModel,P<:AbstractString}
    ...
    size(saved.weights_0) == size(model.weights_0) ||
        throw(ArgumentError("resume checkpoint W0 shape $(size(saved.weights_0)) does not match model $(size(model.weights_0))"))
    size(saved.weights_12) == size(model.weights_12) ||
        throw(ArgumentError("resume checkpoint W12 shape $(size(saved.weights_12)) does not match model $(size(model.weights_12))"))
    size(saved.weights_2o) == size(model.weights_2o) ||
        throw(ArgumentError("resume checkpoint W2O shape $(size(saved.weights_2o)) does not match model $(size(model.weights_2o))"))

    # Radius-specific masks decide which resumed local edges are physically present.
    model.weights_0 .= saved.weights_0 .* model.mask_0
    model.weights_12 .= saved.weights_12 .* model.mask_12
    model.weights_2o .= saved.weights_2o
    model.weights_11 .= saved.weights_11 .* model.mask_11
    model.weights_22 .= saved.weights_22 .* model.mask_22
    model.weights_oo .= saved.weights_oo .* model.mask_oo
...
function local_worker_algorithm(
    ...
    free_temperature = GeometricTemperatureSchedule(; start_T = config.hot_temp, stop_T = config.cold_temp)
    nudge_temperature = ReverseAnnealTemperatureSchedule(; cold_T = config.cold_temp, peak_T = config.reverse_temp)
    return Processes.@Routine begin
        @alias dynamics = dynamics_algorithm
        @alias free_temperature = free_temperature
        @alias nudge_temperature = nudge_temperature
        @state model
        @state x
        @state y
        @state gradient
        ...
        ResetBestEnergyCapture!(best_energy = free_best_energy, best_state = free_state)
        @repeat free_reads begin
            RandomizeGraphState!(isinggraph = dynamics.model, rng = rng)
            install_sample_bias!(dynamics.model, x)
            free_temperature.step_idx = 0
            @repeat free_steps begin
                free_temperature(isinggraph = dynamics.model)
                dynamics()
            end
            CaptureBestEnergyState!(isinggraph = dynamics.model, best_energy = free_best_energy, best_state = free_state)
        end

        ResetBestEnergyCapture!(best_energy = nudged_best_energy, best_state = nudged_state)
        @repeat nudge_reads begin
            SetGraphState!(isinggraph = dynamics.model, target = free_state)
            install_sample_bias!(dynamics.model, x, y, dynamics.model.config.β)
            nudge_temperature.step_idx = 0
            @repeat nudge_steps begin
                nudge_temperature(isinggraph = dynamics.model)
                dynamics()
            end
            CaptureBestEnergyState!(isinggraph = dynamics.model, best_energy = nudged_best_energy, best_state = nudged_state)
        end

        stats = finish_contrastive_sample!(gradient, dynamics.model, x, y, free_state, nudged_state)
...
function local_worker(source::M, worker_idx::I, dynamics_algorithm::D) where {M<:PaperMNISTModel,I<:Integer,D}
    ...
    return Processes.Process(
        ...
        Processes.Init(:free_temperature; n_steps = free_steps),
        Processes.Init(:nudge_temperature; n_steps = nudge_steps);
        repeat = 1,
    )
end
...
function local_manager(source::M) where {M<:PaperMNISTModel}
    ...
    recipe = (;
        makeworker = (idx, manager) -> local_worker(manager.state.model, idx, dynamics_algorithm),
...
function free_phase_process(model::M, dynamics_algorithm::D) where {M<:PaperMNISTModel,D}
    graph_state = II.state(model.graph)
    algorithm = Processes.resolve(free_phase_algorithm(deepcopy(dynamics_algorithm), model.config, length(graph_state)))
    return Processes.Process(
        algorithm,
        Processes.Init(:_state;
            model,
        ...
        Processes.Init(:dynamics; model = model.graph),
        Processes.Init(:free_temperature; n_steps = phase_steps(model, model.config.free_sweeps));
        repeat = 1,
    )
end
...
function plot_metrics(path::P, rows::R) where {P<:AbstractString,R<:AbstractVector}
    ...
    ax_acc = Axis(fig[1, 1], xlabel = "epoch", ylabel = "accuracy", title = "Local MNIST manager accuracy")
...
function plot_diagnostics(path::P, rows::R) where {P<:AbstractString,R<:AbstractVector}
    ...
    ax_time = Axis(fig[1, 1], xlabel = "epoch", ylabel = "seconds", title = "Epoch Time")
    ax_skip = Axis(fig[2, 1], xlabel = "epoch", ylabel = "skipped samples", title = "Skipped Samples")
    lines!(ax_time, [row.epoch for row in timed_rows], [row.seconds for row in timed_rows], color = :steelblue)
    scatter!(ax_time, [row.epoch for row in timed_rows], [row.seconds for row in timed_rows], color = :steelblue)

    skipped_rows = [row for row in timed_rows if !ismissing(row.skipped)]
    if !isempty(skipped_rows)
        lines!(ax_skip, [row.epoch for row in skipped_rows], [row.skipped for row in skipped_rows], color = :orange)
        scatter!(ax_skip, [row.epoch for row in skipped_rows], [row.skipped for row in skipped_rows], color = :orange)
    end
...
function run_config!(config::C) where {C<:PaperMNISTManagerConfig}
    ...
    resume_model!(source, resume_checkpoint_path())
