...
$Root = Join-Path $ScriptDir ("experiments\current\" + $Stamp + "_lcl_6x6_clamp_corrected_beta_temp_grid")
...
function Invoke-LclRun {
    ...
    $name = "lcl6x6_clamp_corrected_b$(Format-Token $Beta)_lr$(Format-Token $LearningRate)_T$(Format-Token $Temp)_ws$(Format-Token $WeightScale)_s$(Format-Token $Sweeps)_train$TrainPerClass`_e$Epochs"
    ...
    if (-not (Test-Path $manifest)) {
        "timestamp,name,nudge,beta,lr,temp,weight_scale,sweeps,epochs,workers,julia_threads,batchsize,train_per_class,test_per_class,outdir" | Out-File -FilePath $manifest -Encoding utf8
    }
    "$(Get-Date -Format s),$name,clamp,$Beta,$LearningRate,$Temp,$WeightScale,$Sweeps,$Epochs,32,32,$BatchSize,$TrainPerClass,$TestPerClass,$outdir" | Out-File -FilePath $manifest -Append -Encoding utf8

    $stdout = Join-Path $Logs "$($name)_stdout.log"
    $stderr = Join-Path $Logs "$($name)_stderr.log"
    "[$(Get-Date -Format s)] START $name nudge=clamp beta=$Beta lr=$LearningRate temp=$Temp weight_scale=$WeightScale sweeps=$Sweeps epochs=$Epochs train_per_class=$TrainPerClass" | Tee-Object -FilePath $stdout -Append
    ...
    if ($proc.ExitCode -ne 0) {
        throw "LCL clamp corrected beta/temp run $name failed with exit code $($proc.ExitCode)"
    }
}

Invoke-LclRun -Beta "0.1" -LearningRate "0.0005" -Temp "0.001" -Sweeps "25" -Epochs "120"
Invoke-LclRun -Beta "0.1" -LearningRate "0.0010" -Temp "0.003" -Sweeps "25" -Epochs "120"
Invoke-LclRun -Beta "0.3" -LearningRate "0.0005" -Temp "0.001" -Sweeps "25" -Epochs "120"
Invoke-LclRun -Beta "0.3" -LearningRate "0.0010" -Temp "0.003" -Sweeps "25" -Epochs "120"
