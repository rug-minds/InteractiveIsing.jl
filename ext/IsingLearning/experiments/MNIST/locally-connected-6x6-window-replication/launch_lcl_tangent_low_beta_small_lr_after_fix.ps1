...
$Root = Join-Path $ScriptDir ("experiments\current\" + $Stamp + "_lcl_6x6_tangent_low_beta_small_lr_after_input_beta_fix")
...
function Invoke-LclRun {
    ...
    $name = "lcl6x6_tangent_lowbeta_afterfix_b$(Format-Token $Beta)_lr$(Format-Token $LearningRate)_T$(Format-Token $Temp)_s$(Format-Token $Sweeps)_train500_e$Epochs"
    ...
    "$(Get-Date -Format s),$name,tangent,$Beta,$LearningRate,$Temp,$Sweeps,$Epochs,32,32,200,500,100,$outdir" | Out-File -FilePath $manifest -Append -Encoding utf8
    ...
    if ($proc.ExitCode -ne 0) {
        throw "LCL tangent low-beta after-fix run $name failed with exit code $($proc.ExitCode)"
    }
}

Invoke-LclRun -Beta "0.1" -LearningRate "0.00001" -Temp "0.0001" -Sweeps "75" -Epochs "160"
Invoke-LclRun -Beta "0.1" -LearningRate "0.00003" -Temp "0.0001" -Sweeps "75" -Epochs "160"
Invoke-LclRun -Beta "0.3" -LearningRate "0.00003" -Temp "0.0001" -Sweeps "75" -Epochs "160"
Invoke-LclRun -Beta "0.3" -LearningRate "0.00010" -Temp "0.0003" -Sweeps "75" -Epochs "160"
