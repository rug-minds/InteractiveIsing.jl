...
$Root = Join-Path $ScriptDir ("experiments\current\" + $Stamp + "_lcl_6x6_tangent_corrected_input_beta_temp_grid_continue")
...
function Invoke-LclRun {
    ...
    if ($proc.ExitCode -ne 0) {
        throw "LCL corrected beta/temp continuation run $name failed with exit code $($proc.ExitCode)"
    }
}
