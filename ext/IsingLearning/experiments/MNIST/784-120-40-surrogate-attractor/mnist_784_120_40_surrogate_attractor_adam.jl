...
Base.@kwdef struct InputFieldMNISTConfig{T<:AbstractFloat,S<:AbstractString}
    ...
    outdir::S = get(
        ...
        joinpath(@__DIR__, "experiments", "current", default_run_dirname("mnist_784_120_40_surrogate_attractor_adam")),
    )
end
...
function write_settings!(path::P, config::C, relaxation_steps::I) where {P<:AbstractString,C<:InputFieldMNISTConfig,I<:Integer}
    open(path, "w") do io
        println(io, "# MNIST 784-120-40 Surrogate-Attractor Adam")
        println(io)
        ...
        println(io, "- learning step: batch-baseline reward covariance over sampled minima")
