...
Base.@kwdef struct InputFieldMNISTConfig{T<:AbstractFloat,S<:AbstractString}
    ...
    outdir::S = get(
        ...
        joinpath(@__DIR__, "experiments", "current", default_run_dirname("mnist_784_120_40_softplus_margin_symmetric_nudge_adam")),
    )
end
...
function write_settings!(path::P, config::C, relaxation_steps::I) where {P<:AbstractString,C<:InputFieldMNISTConfig,I<:Integer}
    open(path, "w") do io
        println(io, "# MNIST 784-120-40 Softplus-Margin Symmetric Nudge Adam Baseline")
        println(io)
...
function main()
    return run_config!(updated_config(InputFieldMNISTConfig(), nudge_mode = "symmetric"))
end
