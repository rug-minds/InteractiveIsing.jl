...
const PMNIST_BOOT_PROGRESS = parse(Bool, lowercase(get(ENV, "ISING_MNIST_2H_PROGRESS", "true")))
...
Base.@kwdef struct LocalMNISTManagerConfig{T<:AbstractFloat,S<:AbstractString}
    name::S = get(ENV, "ISING_MNIST_2H_NAME", "two_hidden_local")
    workers::Int = parse(Int, get(ENV, "ISING_MNIST_2H_WORKERS", "32"))
    epochs::Int = parse(Int, get(ENV, "ISING_MNIST_2H_EPOCHS", "250"))
    batchsize::Int = parse(Int, get(ENV, "ISING_MNIST_2H_BATCHSIZE", "32"))
    train_per_class::Int = parse(Int, get(ENV, "ISING_MNIST_2H_TRAIN_PER_CLASS", "100"))
    test_per_class::Int = parse(Int, get(ENV, "ISING_MNIST_2H_TEST_PER_CLASS", "20"))
    hidden1_side::Int = parse(Int, get(ENV, "ISING_MNIST_2H_H1_SIDE", "11"))
    hidden2_side::Int = parse(Int, get(ENV, "ISING_MNIST_2H_H2_SIDE", "5"))
    output_replicas::Int = parse(Int, get(ENV, "ISING_MNIST_2H_OUTPUT_REPLICAS", "4"))
    input_hidden_radius::Int = parse(Int, get(ENV, "ISING_MNIST_2H_R1", "7"))
    hidden_hidden_radius::Int = parse(Int, get(ENV, "ISING_MNIST_2H_R2", "3"))
    internal_radius::Int = parse(Int, get(ENV, "ISING_MNIST_2H_INTERNAL_RADIUS", "1"))
    output_internal_radius::Int = parse(Int, get(ENV, "ISING_MNIST_2H_OUTPUT_INTERNAL_RADIUS", "1"))
    free_reads::Int = parse(Int, get(ENV, "ISING_MNIST_2H_FREE_READS", "3"))
    nudge_reads::Int = parse(Int, get(ENV, "ISING_MNIST_2H_NUDGE_READS", "3"))
    free_sweeps::Int = parse(Int, get(ENV, "ISING_MNIST_2H_FREE_SWEEPS", "75"))
    nudge_sweeps::Int = parse(Int, get(ENV, "ISING_MNIST_2H_NUDGE_SWEEPS", "75"))
    β::T = parse(PMNIST_FT, get(ENV, "ISING_MNIST_2H_BETA", "5.0"))
    target_on::T = parse(PMNIST_FT, get(ENV, "ISING_MNIST_2H_TARGET_ON", "1.0"))
    target_off::T = parse(PMNIST_FT, get(ENV, "ISING_MNIST_2H_TARGET_OFF", "-1.0"))
    lr_w0::T = parse(PMNIST_FT, get(ENV, "ISING_MNIST_2H_LR_W0", "0.012"))
    lr_w12::T = parse(PMNIST_FT, get(ENV, "ISING_MNIST_2H_LR_W12", "0.012"))
    lr_w2o::T = parse(PMNIST_FT, get(ENV, "ISING_MNIST_2H_LR_W2O", "0.012"))
    lr_w11::T = parse(PMNIST_FT, get(ENV, "ISING_MNIST_2H_LR_W11", "0.003"))
    lr_w22::T = parse(PMNIST_FT, get(ENV, "ISING_MNIST_2H_LR_W22", "0.003"))
    lr_woo::T = parse(PMNIST_FT, get(ENV, "ISING_MNIST_2H_LR_WOO", "0.003"))
    lr_b::T = parse(PMNIST_FT, get(ENV, "ISING_MNIST_2H_LR_B", "0.0012"))
    optimizer::S = lowercase(get(ENV, "ISING_MNIST_2H_OPTIMIZER", "adam"))
    gain_w0::T = parse(PMNIST_FT, get(ENV, "ISING_MNIST_2H_GAIN_W0", "0.5"))
    gain_w12::T = parse(PMNIST_FT, get(ENV, "ISING_MNIST_2H_GAIN_W12", "0.25"))
    gain_w2o::T = parse(PMNIST_FT, get(ENV, "ISING_MNIST_2H_GAIN_W2O", "0.25"))
    gain_w11::T = parse(PMNIST_FT, get(ENV, "ISING_MNIST_2H_GAIN_W11", "0.0"))
    gain_w22::T = parse(PMNIST_FT, get(ENV, "ISING_MNIST_2H_GAIN_W22", "0.0"))
    gain_woo::T = parse(PMNIST_FT, get(ENV, "ISING_MNIST_2H_GAIN_WOO", "0.0"))
    internal_scale::T = parse(PMNIST_FT, get(ENV, "ISING_MNIST_2H_INTERNAL_SCALE", "0.0"))
    output_internal_scale::T = parse(PMNIST_FT, get(ENV, "ISING_MNIST_2H_OUTPUT_INTERNAL_SCALE", "0.0"))
    train_internal::Bool = parse(Bool, lowercase(get(ENV, "ISING_MNIST_2H_TRAIN_INTERNAL", "false")))
    weight_clip::T = parse(PMNIST_FT, get(ENV, "ISING_MNIST_2H_WEIGHT_CLIP", "1.0"))
    bias_clip::T = parse(PMNIST_FT, get(ENV, "ISING_MNIST_2H_BIAS_CLIP", "1.0"))
    applied_bias_clip::T = parse(PMNIST_FT, get(ENV, "ISING_MNIST_2H_APPLIED_BIAS_CLIP", "4.0"))
    hot_temp::T = parse(PMNIST_FT, get(ENV, "ISING_MNIST_2H_HOT_TEMP", "5.0"))
    cold_temp::T = parse(PMNIST_FT, get(ENV, "ISING_MNIST_2H_COLD_TEMP", "0.01"))
    reverse_temp::T = parse(PMNIST_FT, get(ENV, "ISING_MNIST_2H_REVERSE_TEMP", "1.0"))
    gradient_normalization::Symbol = Symbol(get(ENV, "ISING_MNIST_2H_GRADIENT_NORMALIZATION", "mean"))
    progress::Bool = parse(Bool, lowercase(get(ENV, "ISING_MNIST_2H_PROGRESS", "true")))
    progress_every::Int = parse(Int, get(ENV, "ISING_MNIST_2H_PROGRESS_EVERY", "10"))
    seed::Int = parse(Int, get(ENV, "ISING_MNIST_2H_SEED", "2468"))
    outdir::S = get(
        ENV,
        "ISING_MNIST_2H_OUTDIR",
        joinpath(@__DIR__, "experiments", "current", "two_hidden_local_" * Dates.format(now(), "yyyymmdd_HHMMSS")),
    )
end
...
function resume_checkpoint_path()
    return get(ENV, "ISING_MNIST_2H_RESUME_CHECKPOINT", "")
end

"""Construct the configured single-step dynamics algorithm for phase routines."""
function mnist_dynamics_algorithm()
    name = lowercase(strip(get(ENV, "ISING_MNIST_2H_DYNAMICS", "metropolis")))
    if name == "metropolis"
        return II.Metropolis()
    elseif name == "local_langevin"
        stepsize = parse(PMNIST_FT, get(ENV, "ISING_MNIST_2H_LANGEVIN_STEPSIZE", "0.1"))
        adjusted = parse(Bool, lowercase(get(ENV, "ISING_MNIST_2H_LANGEVIN_ADJUSTED", "false")))
        order = Symbol(get(ENV, "ISING_MNIST_2H_LANGEVIN_ORDER", "random"))
    ...
    elseif name == "block_langevin"
        stepsize = parse(PMNIST_FT, get(ENV, "ISING_MNIST_2H_LANGEVIN_STEPSIZE", "0.1"))
        adjusted = parse(Bool, lowercase(get(ENV, "ISING_MNIST_2H_LANGEVIN_ADJUSTED", "false")))
        block_size = parse(Int, get(ENV, "ISING_MNIST_2H_LANGEVIN_BLOCK_SIZE", "256"))
        return II.BlockLangevin(; stepsize, adjusted, block_size)
    end
    throw(ArgumentError("unknown ISING_MNIST_2H_DYNAMICS=`$name`"))
end
...
"""Parse selected `(r1, r2)` pairs from a compact `r1:r2,...` string."""
function parse_radius_pairs(value::S, default::V) where {S<:AbstractString,V<:AbstractVector{<:Tuple{Int,Int}}}
    isempty(strip(value)) && return default
    pairs = Tuple{Int,Int}[]
    for part in split(value, ",")
        stripped = strip(part)
        isempty(stripped) && continue
        coords = split(stripped, ":")
        length(coords) == 2 || throw(ArgumentError("radius pair `$stripped` must look like `r1:r2`"))
        push!(pairs, (parse(Int, strip(coords[1])), parse(Int, strip(coords[2]))))
    end
    return pairs
end
...
"""Construct the sampled input-hidden-hidden-readout Ising graph."""
...
function init_model(config::C, seed::I = config.seed) where {C<:LocalMNISTManagerConfig,I<:Integer}
    ...
    t_masks = time()
    input_hidden_connectivity = local_mask(PMNIST_INPUT_SIDE, config.hidden1_side, config.input_hidden_radius)
    hidden_hidden_connectivity = local_mask(config.hidden1_side, config.hidden2_side, config.hidden_hidden_radius)
...
function optimizer_rule(config::C, lr::T) where {C<:LocalMNISTManagerConfig,T<:Real}
    ...
    throw(ArgumentError("unknown ISING_MNIST_2H_OPTIMIZER=`$(config.optimizer)`; use `adam` or `descent`"))
end
...
function local_manager(source::M) where {M<:LocalMNISTModel}
    ...
    t_resolve = time()
    worker_algorithm = StatefulAlgorithms.resolve(local_worker_algorithm(deepcopy(dynamics_algorithm), source.config, length(II.state(source.graph))))
...
function plot_metrics(path::P, rows::R) where {P<:AbstractString,R<:AbstractVector}
    ...
    ax_acc = CM.Axis(fig[1, 1], xlabel = "epoch", ylabel = "accuracy", title = "Two-hidden local MNIST accuracy")
...
function write_settings!(path::P, config::C) where {P<:AbstractString,C<:LocalMNISTManagerConfig}
    open(path, "w") do io
        println(io, "# Two-Hidden Local MNIST")
        println(io)
        ...
        println(io, "- radii r1/r2: `$(config.input_hidden_radius)` / `$(config.hidden_hidden_radius)`")
...
function run_config!(config::C) where {C<:LocalMNISTManagerConfig}
    ...
    try
        ...
        println("saved two-hidden local MNIST manager run in ", config.outdir)
...
"""Run a selected `(r1, r2)` grid using the ProcessManager two-hidden recipe."""
function main()
    base = LocalMNISTManagerConfig()
    base.workers > 0 || throw(ArgumentError("ISING_MNIST_2H_WORKERS must be positive"))
    base.batchsize > 0 || throw(ArgumentError("ISING_MNIST_2H_BATCHSIZE must be positive"))
    ...
    root_outdir = haskey(ENV, "ISING_MNIST_2H_OUTDIR") ? base.outdir :
        joinpath(@__DIR__, "experiments", "current", "r1_r2_pairs_e$(base.epochs)_" * Dates.format(now(), "yyyymmdd_HHMMSS"))
    results = NamedTuple[]
    for (r1, r2) in pairs
        name = "r1_$(r1)_r2_$(r2)"
        outdir = joinpath(root_outdir, name)
        config = copy_config(base; name, input_hidden_radius = r1, hidden_hidden_radius = r2, outdir)
