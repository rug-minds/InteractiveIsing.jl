...
include(joinpath(@__DIR__, "..", "checkerboard-local-cnn-two-hidden-layers", "xor_local_cnn_like_grid.jl"))
...
function main()
    ...
    outdir = get(
        ENV,
        "ISING_XOR_DIAG_OUTDIR",
        joinpath(@__DIR__, "runs", "manager", "xor_local_manager_diagnostics_" * Dates.format(now(), "yyyymmdd_HHMMSS")),
    )
    mkpath(outdir)
