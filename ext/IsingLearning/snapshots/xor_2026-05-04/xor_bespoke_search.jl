...
function readout_vector(output_units::Integer)
    return output_pattern(true, output_units) .- output_pattern(false, output_units)
end

function readout_score(output, output_units::Integer)
    return dot(readout_vector(output_units), output)
end

function label_target_score(label::Bool, config)
    if config.target_mode === :pattern
        return readout_score(output_pattern(label, config.output_units), config.output_units)
...
function output_clamping_target(label::Bool, config)
    if config.target_mode === :pattern
        return output_pattern(label, config.output_units)
    elseif config.target_mode === :readout
        w = readout_vector(config.output_units)
...
function xor_architecture(config)
    ...
    clamping =
        config.target_mode === :readout_hterm ?
        LinearReadoutClamping(output_idxs, readout_vector(config.output_units); β = zero(FT), target = zero(FT)) :
        config.target_mode === :constant_readout_hterm ?
        ConstantLinearReadoutNudge(output_idxs, readout_vector(config.output_units); β = zero(FT), target = zero(FT), free_score = zero(FT)) :
...
function evaluate_xor!(trainer, x, y, cases, config; seed_offset::Integer = 10_000)
    ...
    score_mse = sum(eachindex(outputs)) do idx
        score = readout_score(outputs[idx], config.output_units)
        target = label_target_score(truth[idx], config)
        abs2(score - target)
    end / length(outputs)
    predictions = map(outputs) do out
        readout_score(out, config.output_units) > zero(FT)
    end
    accuracy = count(==(true), predictions .== truth) / length(truth)
    margin = minimum(abs(readout_score(outputs[idx], config.output_units)) for idx in eachindex(outputs))
...
for (idx, config) in enumerate(configs)
    ...
    if PRINT_OUTPUTS
        for (case, out) in zip(((false, false), (false, true), (true, false), (true, true)), result.after.outputs)
            score = readout_score(out, result.config.output_units)
