...
include(joinpath(@__DIR__, "output_state_2_4_1_timeavg_learning.jl"))
...
already reached low MSE without time-averaged state capture.
"""
...
`train_avg` values test how many full-state samples to average in each nudged
phase before computing the normal EqProp contrastive gradient.
"""
function grid_specs()
    base = (
        β = FT(2.0),
