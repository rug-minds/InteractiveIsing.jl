...
# Snapshot of the first XOR setup that reliably learned in this branch.
#
# Keep the environment overrides so the file remains easy to probe, but the
# defaults below are the exact settings from the successful 1000-epoch run
# documented in docs/src/xor_successful_learning.md.
const EPOCHS = parse(Int, get(ENV, "ISING_XOR_SUCCESS_EPOCHS", "1000"))
const LOG_EVERY = parse(Int, get(ENV, "ISING_XOR_SUCCESS_LOG_EVERY", "100"))
const MINIT = parse(Int, get(ENV, "ISING_XOR_SUCCESS_MINIT", "4"))
const EVAL_REPEATS = parse(Int, get(ENV, "ISING_XOR_SUCCESS_EVAL_REPEATS", "16"))
const HIDDEN_UNITS = parse(Int, get(ENV, "ISING_XOR_SUCCESS_HIDDEN", "16"))
const OUTPUT_UNITS = 2
const INPUT_UNITS = 4
const BETA = parse(FT, get(ENV, "ISING_XOR_SUCCESS_BETA", "0.05"))
const LEARNING_RATE = parse(FT, get(ENV, "ISING_XOR_SUCCESS_LR", "0.005"))
const WEIGHT_DECAY = parse(FT, get(ENV, "ISING_XOR_SUCCESS_WEIGHT_DECAY", "1e-4"))
const WEIGHT_SCALE = parse(FT, get(ENV, "ISING_XOR_SUCCESS_WEIGHT_SCALE", "0.03"))
const BIAS_SCALE = parse(FT, get(ENV, "ISING_XOR_SUCCESS_BIAS_SCALE", "0.1"))
const TEMP = parse(FT, get(ENV, "ISING_XOR_SUCCESS_TEMP", "0.001"))
const STEPSIZE = parse(FT, get(ENV, "ISING_XOR_SUCCESS_STEPSIZE", "0.05"))
const BLOCK_SIZE = parse(Int, get(ENV, "ISING_XOR_SUCCESS_BLOCK_SIZE", "8"))
const FREE_RELAXATION = parse(Int, get(ENV, "ISING_XOR_SUCCESS_FREE_RELAXATION", "50"))
const NUDGED_RELAXATION = parse(Int, get(ENV, "ISING_XOR_SUCCESS_NUDGED_RELAXATION", "50"))
const WEIGHT_SEED = parse(Int, get(ENV, "ISING_XOR_SUCCESS_WEIGHT_SEED", "2"))
const BIAS_SEED = parse(Int, get(ENV, "ISING_XOR_SUCCESS_BIAS_SEED", "11"))
const BASE_SEED = parse(Int, get(ENV, "ISING_XOR_SUCCESS_BASE_SEED", "62000"))
const EVAL_SEED_OFFSET = parse(Int, get(ENV, "ISING_XOR_SUCCESS_EVAL_SEED_OFFSET", "30000000"))
const OUTDIR = get(
    ENV,
    "ISING_XOR_SUCCESS_DIR",
    joinpath(@__DIR__, "..", "runs", "xor_succesful_learning_" * Dates.format(now(), "yyyymmdd_HHMMSS")),
)
...
function main()
    ...
    println(
        "Running successful XOR learning snapshot: ",
    ...
    csv_path = write_csv(joinpath(OUTDIR, "xor_succesful_learning.csv"), rows)
    png_path = plot_metrics(joinpath(OUTDIR, "xor_succesful_learning.png"), rows)
