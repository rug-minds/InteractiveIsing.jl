...
full_process = Forwards_and_Nudged(lux_model)

fp = resolve(full_process.algorithm)
p = InlineProcess(fp, Input(:_state; buffers, equilibrium_state = copy(state(rbm))), Input(:dynamics, state = rbm), Input(:plus_capture, state = rbm), Input(:minus_capture, state = rbm); repeats = 1)
rc = run(p)
@benchmark run(p)
@code_warntype run(p)