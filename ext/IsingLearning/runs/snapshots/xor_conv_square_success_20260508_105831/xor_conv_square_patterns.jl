...
function internal_nn_weight_generator(seed::Integer)
    rng = Random.MersenneTwister(seed)
    return II.WeightGeneratorOld(dr -> INTERNAL_WEIGHT_SCALE * randn(rng, FT), INTERNAL_NN, rng)
end
...
function xor_graph()
    input_wg = internal_nn_weight_generator(INTERNAL_WEIGHT_SEED)
    hidden_wgs = [internal_nn_weight_generator(INTERNAL_WEIGHT_SEED + idx) for idx in 1:HIDDEN_LAYERS]
    output_wg = internal_nn_weight_generator(INTERNAL_WEIGHT_SEED + HIDDEN_LAYERS + 1)
    layers = Any[
